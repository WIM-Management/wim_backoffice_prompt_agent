...
`wim-backoffice-prompt-agent`는 각 개발자 머신에서 백그라운드로 돌며, **Claude Code의 로컬 세션 파일**(`~/.claude/projects/**/*.jsonl`)에서 완결된 대화 턴(프롬프트 + 응답)을 주기적으로 읽습니다. 시크릿을 마스킹하고, 로컬 디스크 큐에 영속한 뒤, 배치로 WIM 백엔드(`PATCH /api/v1/prompt-insights/events`)에 업로드합니다.
